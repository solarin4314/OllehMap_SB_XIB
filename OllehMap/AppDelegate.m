...
//        parser = [[URLParser alloc] initWithURLString:url.absoluteString];
    ...
    @try
    {
        ...
        else if (ptype && [ptype isEqualToString:@"m_route"])
        {
            OllehMapStatus *oms = [OllehMapStatus sharedOllehMapStatus];
            ...
            Coord startCoordnate = CoordMake(946397.198447, 1943199.079029);
            [oms.searchResultRouteStart setIsCurrentLocation:NO];
            [oms.searchResultRouteStart setStrLocationName:@"믹시스템 테스트"];
            ...
            Coord destCoordinate = CoordMake(958246, 1944403);
            [oms.searchResultRouteDest setIsCurrentLocation:NO];
            [oms.searchResultRouteDest setStrLocationName:@"강남역 테스트"];
            [oms.searchResultRouteDest setCoordLocationPoint:destCoordinate];
            
            NSString *searchType = [parser valueForVariable:@"searchType"];
            int option = [[parser valueForVariable:@"option"] intValue];
            int optionType = 0;
            ...
            switch (option)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    optionType = SearchRoute_Car_SearchType_Optimal;
                    break;
                default:
            ...
            if([searchType isEqualToString:@"car"])

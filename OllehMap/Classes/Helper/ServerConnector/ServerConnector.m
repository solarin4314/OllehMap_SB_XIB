...

{
    ...
    NSString *keyString = (NSString *)CFURLCreateStringByAddingPercentEscapes(NULL, (CFStringRef)key, NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
...
- (ServerRequester *) requestSearchPublicBusStation :(id)target action:(SEL)action Name:(NSString *)Name ViewCnt:(int)ViewCnt Page:(int)Page
{
    ServerRequester *request = [[[ServerRequester alloc] init] autorelease];
  	...
  	NSString *keyString = (NSString *)CFURLCreateStringByAddingPercentEscapes(NULL, (CFStringRef)Name, NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8);
...
- (ServerRequester *)requestSearchURL:(id)target action:(SEL)action PX:(int)PX PY:(int)PY Query:(NSString *)query SearchType:(NSString *)searchType order:(NSString *)order
{
    ...
    NSString *requestLongURL = [NSString stringWithFormat:
                                @"%@ptype=m_search&x=%d&y=%d&query=%@&searchtype=%@&order=%@&coordtype=utmk",COMMON_URL_SHARE,PX, PY,(NSString *)CFURLCreateStringByAddingPercentEscapes(NULL, (CFStringRef)query, NULL, (CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8), searchType, order];
...
- (void) finishSearchRoadSearch:(id)request
{
    if ([request finishCode] == OMSRFinishCode_Completed)
	{
        ...
        @try
        {
            ...
            [[OllehMapStatus sharedOllehMapStatus].roadNameDictionary removeAllObjects];
            
            [[OllehMapStatus sharedOllehMapStatus].roadNameDictionary setObject:[dic objectForKeyGC:@"RESULTDATA"] forKey:@"RoadCode"];
            //[[OllehMapStatus sharedOllehMapStatus].linePolygonArray addObjectsFromArray:[[[[[dic objectForKeyGC:@"RESULTDATA"] objectAtIndexGC:0] objectForKeyGC:@"part"] objectAtIndexGC:0] objectForKeyGC:@"vertex"]];

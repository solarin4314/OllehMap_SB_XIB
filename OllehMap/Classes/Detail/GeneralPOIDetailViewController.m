...
@synthesize addrNewStr = _addrNewStr;
...
- (void) drawTopView
{
    ...
    if(![self.addrNewStr isEqualToString:@""])
    {
        UILabel *poiNewAdd = [[UILabel alloc] init]; 
        [poiNewAdd setText:self.addrNewStr];
        ...
        //CGSize newAddSize = [self.addrNewStr sizeWithFont:poiNewAdd.font constrainedToSize:CGSizeMake(183, FLT_MAX)];
        
        [poiNewAdd setFrame:CGRectMake(127, poiAdd.frame.origin.y + poiAdd.frame.size.height + 6 - 1, 183, 13)];
        
        // 서브주소이미지
        UIImageView *subImg = [[UIImageView alloc] init];
        [subImg setImage:[UIImage imageNamed:@"new_address_icon.png"]];
        [subImg setFrame:CGRectMake(90, poiAdd.frame.origin.y + poiAdd.frame.size.height + 6 - 1, 33, 15)];
...
{
    //[_callLinkAlertView setFrame:CGRectMake(37, 47, 410/2, 390/2)];
    [_callLinkAlertView setFrame:CGRectMake(116/2, 284/2 - 88, 410/2, 390/2)];
}
// 무료통화 걸기

...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if ( ![nibNameOrNil isEqualToString:@"MainMapViewController"] )
    {
        ...
        else if ([nibNameOrNil isEqualToString:@"MainMapViewController_SearchResult_SinglePOI_Line"])
        {
            _nMapRenderType = MapRenderType_SearchResult_SinglePOI;
            _nMapRednerSinglePOICategory = MainMap_SinglePOI_Type_Line;
        }
        // 멀티 POI 처리
        ...
        else if ([nibNameOrNil isEqualToString:@"MainMapViewController_SearchResult_RoadName"])
        {
...
+ (void) markingSinglePOI_RenderType:(int)type animated:(BOOL)animated
{
    ...
    else if (type == MainMap_SinglePOI_Type_Line)
    {
        mmvc = [[MainMapViewController alloc] initWithNibName:@"MainMapViewController_SearchResult_SinglePOI_Line" bundle:nil];
    }
    else
    {
...
+ (void) markingRoadCodeSearch:(NSString *)keyword animated:(BOOL)animated
{
    // 뷰 컨트롤러 생성
    MainMapViewController *mmvc = [[MainMapViewController alloc] initWithNibName:@"MainMapViewController_SearchResult_RoadName" bundle:nil];
...
- (void) markingSinglePOI
{
    ...
    else if (_nMapRednerSinglePOICategory == MainMap_SinglePOI_Type_Line)
    {
...
- (void) markingRoadName
{
    ...
    int vertexCnt = [[[[oms.roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
    ...
    if ( [overlay isKindOfClass:[OMImageOverlay class]] )
    {
        ...
        else if ( [overlay isMemberOfClass:[OMImageOverlayTheme class]] )
        {
            ...
            else
            {
                ...
                {
                    [self showDuplicatedPOIList:themeOverlay title:@"테마결과"];
                }
                // 단일 POI 선택한 경우
                else
                {
...
- (void) mapStatusChanged:(NSNumber *)mapLoad isZoom:(NSNumber *)isZoom
{
    ...
    if ( [mapLoad intValue] == 2 )
    {
        ...
        else if (_nMapRenderType == MapRenderType_SearchResult_RoadCode)
        {
            ...
            int vertextCnt = [[[[[OllehMapStatus sharedOllehMapStatus].roadNameDictionary objectForKeyGC:@"RoadCode"] objectAtIndexGC:0] objectForKeyGC:@"part"] count];
            ...
            [self pinSearchSinglePOIOverlayRoadName:YES modiX:shortX modiY:shortY];
        }
...
- (void) pinSearchSinglePOIOverlayRoadName:(BOOL)isDisplay modiX:(float)x modiY:(float)y
{
    // 검색결과 단일 POI 처리
    ...
    if (isDisplay)
    {
        ...
        [overlay.additionalInfo setObject:[NSNumber numberWithFloat:x] forKey:@"X"];
        [overlay.additionalInfo setObject:[NSNumber numberWithFloat:y] forKey:@"Y"];
        [overlay.additionalInfo setObject:oms.searchResult.strRdCd forKey:@"RdCd"];
        // 마커옵션 자동으로 펼치기
        [self pinPOIMarkerOption:YES targetInfo:overlay.additionalInfo animated:NO];

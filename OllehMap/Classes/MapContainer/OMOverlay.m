...
- (void) setSelected:(BOOL)selected
{
    ...
    if (selected)
    {
        ...
        {
            if(_lineCcoordY == 0)
                _lineCcoordY = marker.frame.origin.y;
            
            [marker setFrame:CGRectMake(marker.frame.origin.x, _coordY - 2, marker.frame.size.width, marker.frame.size.height)];
        }
    ...
    else
    {
        ...
        for (UIImageView *marker in self.getOverlayView.subviews)
        {
            [marker setFrame:CGRectMake(marker.frame.origin.x, _coordY, marker.frame.size.width, marker.frame.size.height)];
        }
    }
    ...
    NSString *imageName = [NSString stringWithFormat:@"%@map_b_marker%@%@.png", line, imageName_POI, imageName_Selected];
    UIImage *poiIconImage = [UIImage imageNamed:imageName];
    [self.imageView setImage:poiIconImage];
    [self setCenterOffset:CGPointMake(self.imageSize.width/2, self.imageSize.height)];
}
...
- (void) setSelected:(BOOL)selected
{
    ...
    if (selected)
    {
        ...
        {
            if(_m_coordY == 0)
                _m_coordY = marker.frame.origin.y;
            
            [marker setFrame:CGRectMake(marker.frame.origin.x, _m_coordY - 2, marker.frame.size.width, marker.frame.size.height)];
        }
    }
    else
    {
        ...
        if([[self.additionalInfo objectForKeyGC:@"Index"] intValue] == indexer)
        {
            for (UIImageView *marker in self.getOverlayView.subviews)
            {
                [marker setFrame:CGRectMake(marker.frame.origin.x, _m_coordY, marker.frame.size.width, marker.frame.size.height)];
            }
        }

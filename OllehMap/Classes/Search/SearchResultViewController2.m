...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if([tableView isKindOfClass:[UIAddressTableView class]])
    {
        ...
        else if (_addressType == addressSearchType_new)
        {
            ...
            NSLog(@"poiDic : %@", [[OllehMapStatus sharedOllehMapStatus].searchLocalDictionary objectForKeyGC:@"DataNewAddress"]);
...
- (void) finishedPolygonInfo:(id)request
{
    if([request finishCode] == OMSRFinishCode_Completed)
    {
        if([[[OllehMapStatus sharedOllehMapStatus].linePolygonDictionary objectForKeyGC:@"LinePolygon"] count] == 0)
        {
            // SinglePOI 렌더링
            [MainMapViewController markingSinglePOI_RenderType:MainMap_SinglePOI_Type_Line animated:NO];
        }
        else
...
- (void)clickToSinglePOIAddress:(id)sender
{
    ...
    [oms.searchResult setIndex:i+1];
    [oms.searchResult setStrRdCd:roadCode];
...
- (void) finishRoadNameSearchInfo:(id)request
{
    if([request finishCode] == OMSRFinishCode_Completed)
    {
        if([[[OllehMapStatus sharedOllehMapStatus].roadNameDictionary objectForKeyGC:@"RoadCode"] count] == 0)
        {
            // SinglePOI 렌더링
            [MainMapViewController markingSinglePOI_RenderType:MainMap_SinglePOI_Type_Line animated:NO];
        }
        ...
        {
            [MainMapViewController markingRoadCodeSearch:@"도로명" animated:NO];
        }
    }
}
...
{
    UIControl* cell = (UIControl*)sender;
    [cell setBackgroundColor:convertHexToDecimalRGBA(@"f4", @"f4", @"f4", 1.0)];
}
